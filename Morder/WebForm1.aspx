<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="WebForm1.aspx.cs" Inherits="Morder.WebForm1" %>
<style type="text/css">
    .auto-style1 {
        width: 133%;
        height: 347px;
                ...
                <table class="auto-style1">
                    ...
                    <tr>
                        <td>
                                ...
                                <br />
                                <br />
                                <asp:DropDownList ID="DdlState" Width="135px" runat="server" DataSourceID="SqlDataSourceState" DataTextField="situation_name" DataValueField="situation_name" AutoPostBack="True" OnSelectedIndexChanged="DdlState_SelectedIndexChanged" OnDataBound="DdlState_DataBound">
                                </asp:DropDownList>
...
<asp:Timer ID="Timer1" runat="server" Interval="3600" ontick="Timer1_Tick"></asp:Timer>
 ...
 <asp:GridView ID="OrderList" runat="server" AutoGenerateColumns="False" DataTextField="Order_Id" DataValueField="Order_Id"  Height="292px" Width="829px" DataKeyNames="Order_Id" DataSourceID="SqlDataSourceOrders" BackColor="White" BorderColor="#DEDFDE" BorderStyle="None" BorderWidth="1px" CellPadding="4" ForeColor="Black" GridLines="Vertical" OnSelectedIndexChanged="OrderList_SelectedIndexChanged" >
                                ...
                                <Columns>
                                    ...
                                    <asp:BoundField DataField="Total_Price" HeaderText="Toplam Fiyat" SortExpression="Total_Price" Visible="False" />
                            ...
                            <asp:SqlDataSource ID="sqldsSituation" runat="server" ConnectionString="<%$ ConnectionStrings:MorderConnectionString %>" SelectCommand="select O.Order_Id,  P.Payment_Type, C.Customer_Name + C.Customer_Surname [Customer Name], O.Total_Price,O.Order_Date, O.Order_Time, O.Order_Note, O.Arrival_Time  from [Order] O  inner join Order_State OS on O.Situation_Id=OS.Situation_Id  inner join Customer C on O.Customer_Id=C.Customer_Id inner join Payment P on O.Payment_Type_Id= P.Payment_Type_Id where O.Enterprise_Id=@Id and O.Address_Id=0 and O.Situation_Id=@sId">
                                <SelectParameters>
                                    <asp:SessionParameter Name="Id" SessionField="EnterpriseId" />
                                    <asp:SessionParameter Name="sId" SessionField="Index" />
                                </SelectParameters>
                            </asp:SqlDataSource>
                            <asp:SqlDataSource ID="SqlDataSourceOrders" runat="server" ConnectionString="<%$ ConnectionStrings:MorderConnectionString %>" SelectCommand="SELECT O.Order_Id, P.Payment_Type, C.Customer_Name + C.Customer_Surname AS [Customer Name], O.Total_Price, O.Order_Date, O.Order_Time, O.Order_Note, O.Arrival_Time FROM [Order] AS O INNER JOIN Order_State AS OS ON O.Situation_Id = OS.Situation_Id INNER JOIN Customer AS C ON O.Customer_Id = C.Customer_Id INNER JOIN Payment AS P ON O.Payment_Type_Id = P.Payment_Type_Id WHERE (O.Enterprise_Id = @id) AND (O.Address_Id = 0) AND (O.Arrival_Time IS NOT NULL) AND (O.Payment_Type_Id &lt;&gt; 6)" DeleteCommand="DELETE FROM [Order] FROM [Order] INNER JOIN Order_State ON [Order].Situation_Id = Order_State.Situation_Id INNER JOIN Customer ON [Order].Customer_Id = Customer.Customer_Id INNER JOIN Customer_Address ON Customer.Customer_Id = Customer_Address.Customer_Id INNER JOIN Address ON Customer_Address.Address_Id = Address.Address_Id INNER JOIN Payment ON [Order].Payment_Type_Id = Payment.Payment_Type_Id">
                                <SelectParameters>
                                    <asp:SessionParameter Name="id" SessionField="EnterpriseId" />
                                </SelectParameters>
                            </asp:SqlDataSource>
